...
const VideoCard: React.FC<Props> = ({ video, layout = 'grid' }) => {
    ...
    return (
      ...
      >
        ...
        <div className="flex flex-col justify-center flex-1">
          ...
          <p className="text-xs text-gray-400 mb-2">{video.type} • {video.year}</p>
          {video.overview && <p className="text-xs text-gray-500 line-clamp-2">{video.overview}</p>}
        </div>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <h3 className="text-sm font-medium text-gray-200 line-clamp-1">{video.title}</h3>
