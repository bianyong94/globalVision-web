...
const Profile = () => {
  ...
  const handleAiResultClick = (keyword: string) =>
          ...
          <form onSubmit={handleAskAI} className="relative z-10">
            <div className="relative">
              <input
                ...
                placeholder="描述剧情..."
                className="w-full bg-black/30 border border-white/10 rounded-xl pl-4 pr-12 py-3 text-sm text-white focus:border-purple-500 transition-all"
              />
              <button
                type="submit"
                disabled={aiLoading || !aiPrompt}
                className="absolute right-2 top-2 p-1.5 bg-purple-600 rounded-lg text-white disabled:opacity-50"
              >
                {aiLoading ? (
          ...
          {aiResults.length > 0 && (
            <div className="mt-4 flex flex-wrap gap-2 relative z-10">
              {aiResults.map((res, idx) => (
                <button
                  key={idx}
                  onClick={() => handleAiResultClick(res)}
                  className="flex items-center gap-1 bg-white/5 border border-white/10 px-3 py-1.5 rounded-full text-xs text-gray-300 hover:text-white transition-all"
                >
                  <SearchIcon size={10} />
                  {res}
                </button>
